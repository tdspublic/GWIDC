...
- HUBSYNC - Commands the GWIDC to send a formatted JSON message to the
  home automation hub IP and port (if configured), notifying the hub
  that the device is requesting to be sychronised.
...
void setup() {
  ...
  server.on("/HUBSYNC", handleHUBSYNC);
  ...
  if (hub_validated) handleHUBSYNC();
}
...
void handlePOLL() {
  ...
  // If requesting IP is set, then also post message to the requesting IP
  // Otherwise this is GWIDC bootup condition and there is no requesting IP
...
void handleHUBSYNC() {
  String message = "============ Called handleHUBSYNC()";
  IPAddress remoteIp = server.client().remoteIP(); // Get the IP address of the client
  ...
  // NEOPIXEL Indicate HUBSYNC state (blue pixels at fixed intervals around the NEOPIXEL)
  for (int i = 0; i < NUM_PIXELS; i++) {
    if ((i % FIXED_PIXEL_INTERVAL) == 0) {
  ...
  // Call POLL(), which will send JSON notification to the hub URL and also 
