...
#define BUILDVERSION "GWIDC-v2-00"
...
- FLASHER - Flashing effect using the current color setting.
- PULSING - Pulsating effect using the current color setting.
...
- BRIGHTNESS=<value> - Sets the overall brightness of the Neopixel
  ring (value must be >= 0 and <= 255)
- SPEED=<value> - Sets the "speed" of the display mode effect 
  for modes other than SOLID and PATTERN
  where 1 = slowest and 5 = fastest (value must be >= 1 and <= 5)
...
/// Declare global constants and variables for managing pulsing patterns in the main loop
...
{ {1,0},             // constant for SOLID, PATTERN, RAINBOW
  {1,1,0},           // even tempo for FLASHER, PULSE, TRAIL
...
// been tested visually to ensure the effects (e.g., FLASHER) are neither too fast nor too 
// slow to be useful or pleasing
const int NUM_SPEEDS = 5;  // Number of speeds contained in the following lookup tables
const int msptu_for_FLASHER[] = {1200,800,400,200,100};
const int msptu_for_PULSING[] = {120,90,60,30,15};
...
void loop() {
  ...
  if ( ((current_Time - start_Time) > pattern[timing_Pattern][step]*ms_per_unit) || flag_pattern_restart ) {
    ...
    else if (strcmp(indicator_Mode,"FLASHER") == 0) { 
      strip.setBrightness(strip_Brightness);
    ...
    else if (strcmp(indicator_Mode,"PULSING") == 0) { 
...
void handlePOLL() {
  ...
  doc["Brightness"] = strip_Brightness;
  doc["Speed"]      = speed_Setting;
  doc["Tone"]       = piezo_Active;
...
void handleREPORT_SETTINGS() {
  ...
  message = message + "Pixel Pattern: " + pixel_Pattern + "<br>";
  message = message + "Brightness: "   + strip_Brightness + "<br>";
...
void handleRoot() {
  ...
  if (server.hasArg("PATTERN")) {
    ...
    if (argument_value.length() == NUM_PIXELS) {
      ...
      // Set the pixel colors here in order to use the function setAverageColorHex
      //to set LEX_HEX_COLOR to the average color across all pixels
      for(int i=0; i< NUM_PIXELS; i++){
        if      (pixel_Pattern[i] == 'O') strip.setPixelColor(i,0,0,0);
        else if (pixel_Pattern[i] == 'R') strip.setPixelColor(i,255,0,0);
        else if (pixel_Pattern[i] == 'G') strip.setPixelColor(i,0,255,0);
      ...
      setAverageColorHex(led_hex_Color);
      ...
      Serial.print(" with average led_hex_Color ");
  ...
  else if (server.hasArg("FLASHER")) {
    // Flash with a single color (hexidecimal value) 
    String message = "Received FLASHER";
    ...
    strcpy(indicator_Mode, "FLASHER");
    timing_Pattern = 1;  // use the timing pattern for FLASHER

    // Check to see if "DEFAULT" was specified for this mode
    // if so, set the mode to a white flasher at medium speed without piezo
    if (server.arg("FLASHER") == "DEFAULT") {
      strcpy(led_hex_Color,"FFFFFF");
      strip_Brightness = 63;
      speed_Setting = 2; // medium speed
      ms_per_unit = msptu_for_FLASHER[speed_Setting]; 
      piezo_Active = false;
    }
    ...
    Serial.print("Set to ");
    Serial.print(indicator_Mode);
    Serial.print(" with led_hex_Color ");
    Serial.print(led_hex_Color);
    Serial.println(". Checking for additional parameters.");
    #endif
  } 
    ...
    strcpy(indicator_Mode, "PULSING");
    timing_Pattern=1; // use the timing pattern for PULSING

    // Check to see if "DEFAULT" was specified for this mode
    // if so, set the mode to a white pulse at medium speed without piezo
    if (server.arg("PULSING") == "DEFAULT") {
      strcpy(led_hex_Color,"FFFFFF");
      strip_Brightness=63;
      speed_Setting = 2; // medium speed
      ms_per_unit = msptu_for_PULSING[speed_Setting]; 
      piezo_Active = false;
    }
    ...
    // Set the pattern string to all 'X's, but with 'W's at the strobing pixels
    for (int i = 0; i < NUM_PIXELS; i++) {
      if ((i % FIXED_PIXEL_INTERVAL) == 0) {
  ...
  if (server.hasArg("BRIGHTNESS")) {
    ...
    strip_Brightness= min(max(int(server.arg("BRIGHTNESS").toInt()),0),255); //Range is 0 to 255
    
    String message = "Received BRIGHTNESS = " + server.arg("BRIGHTNESS");
 ...
 if (server.hasArg("SPEED")) {
    ...
    speed_Setting = min(max(int(server.arg("SPEED").toInt()),0),NUM_SPEEDS-1); //Range is 0 to NUM_SPEEDS-1

    if      (strcmp(indicator_Mode,"FLASHER") == 0) ms_per_unit =  msptu_for_FLASHER[speed_Setting]; 
    else if (strcmp(indicator_Mode,"PULSING") == 0) ms_per_unit =  msptu_for_PULSING[speed_Setting]; 
...
void handleSTARTUP() {
  ...
  for (int i = 0; i < NUM_PIXELS; i++) {
    if ((i % FIXED_PIXEL_INTERVAL) == 0) {
      strip.setPixelColor(i, 0, 0, pixel_brightness);
      pixel_Pattern[i]='B';
      }
    else {
      strip.setPixelColor(i, 0, 0, 0);
  ...
  strip.show();   // Indicate GWIDC device is ready to communicate with Hubitat
                  // by showing 4 blue pixels
  strcpy(led_hex_Color,"000010");
...
void setAverageColorHex(char* hexColorBuffer) {
...
//NOTE to SELF - unclear why this is needed.  
