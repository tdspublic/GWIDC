...
#define BUILDVERSION "GWIDC_v2_52"
...
void setup() {
  ...
  //Initialize reamaing global parameters used to communicate the GWIDC device status
...
void loop() {
  ...
  if ( ((current_Time - start_Time) > pattern[timing_Pattern][step]*ms_per_unit) || flag_pattern_restart ) {
    ...
    if (strcmp(mode_Param,"RAINBOW") == 0) {
      ...
      strip.setBrightness(strip_Brightness);
      if (step % 2 == 0) setcolor_Param (color_Param);
      else setcolor_Param ("000000");
      strip.show();
    }
...
bool setcolor_Param(const char *data) {
  int red, green, blue;
  int result = sscanf(data, "%02x%02x%02x", &red, &green, &blue);
  if (result == 3) {
