...
// Compatible with GWIDC Firmware v2_50

// SPDX-License-Identifier: MIT
// Copyright (c) 2025 Timothy Sakulich

metadata {
    definition (name: "GWIDC_Hubitat_v2_50", namespace: "tjsakulich", author: "Timothy Sakulich") {
        capability "Actuator"
        capability "Alarm"
        capability "Bulb"        
    ...
    preferences {
        section("GWIDC Settings") {
	        ...
	        input("GWIDC_Strobe_Color", "string", title: "RGB Color for 'Strobe' (hex)", required: false)
	    }   
   } 
}
...
def installed() {
   ...
   GWIDC_On = "PULSE&RGB=FF0000&LEVEL=50&SPEED=2&TONE=0"
   GWIDC_Off = "PATTERN=GOOOGOOOGOOO&LEVEL=1&SPEED=0&TONE=0"
...
//====================================================
def both() {
    CommandMessage = "?STROBE&RGB=${GWIDC_Strobe_Color}&LEVEL=100&SPEED=2&TONE=1"
...
//====================================================
def siren() {
    CommandMessage = "?STROBE&RGB=000000&LEVEL=0&SPEED=2&TONE=1"
...
//====================================================
def strobe() {
    CommandMessage = "?STROBE&RGB=${GWIDC_Strobe_Color}&LEVEL=100&SPEED=2&TONE=0"
    ...
    if (response.getStatus() == 200) {
        try {
    		...
    		// and then calls initialize() 
   			if (jsonResponse.Mode == "STARTUP") {
     		  if (EnableDebug) log.debug "Device reported STARTUP State to Hubitat"
       		initialize()
    		}
        } catch (e) {
...
// up and sends a JSON message containing "STARTUP" as the device display mode. 
...
def setHue(value) {
    ...
    Current_RGBvalue = device.currentValue("Color")
